...
class Simulation:
    ...
    def bounceBackFluid(self):
        for x, y in self.wallBoundary.boundaryIndex:
            for latticeIndex in range(latticeSize):
                if (self.fluid[x, y, latticeIndex] != 0):
                    if latticeIndex in [3, 4, 7, 8]:
                        reflectIndex = latticeIndex - 2
                    else:
                        reflectIndex = latticeIndex + 2
                    self.fluid[x - Simulation.unitX[latticeIndex], y - Simulation.unitY[latticeIndex], reflectIndex] = self.fluid[x, y, latticeIndex]
                    self.fluid[x, y, latticeIndex] = 0
        self.updateSpeed()
